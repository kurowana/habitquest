    ...
    <div>
        ...
        <table><tbody>
            ...
            <tr v-for="habit in getHasHabit" :key="habit.id">
                ...
                <td>{{habit.count}}回</td>
                <td><button @click="habitPlus(habit.id)">活動報告</button></td>
                <td><button @click="habitDelete(habit.id)">削除</button></td>
            </tr>
        </tbody></table>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        habitPlus:function(habit_id){
            axios.post('./api/add_habit_count',{
                user_id:this.$store.state.user.id,
                habit_id:habit_id
            }).then((res)=>{
                location.reload()
